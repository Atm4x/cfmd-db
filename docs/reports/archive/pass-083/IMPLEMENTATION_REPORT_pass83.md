...
# Pass83 implementation status — unified advisor telemetry and recovery economics

Pass83 adds reconstructible deterministic advisor telemetry and a stateful production `UnifiedAdvisorController` on top of the Pass82 shared candidate/resource/capability substrate. Read work saved, write-maintenance work and rebuild work feed the existing common selector; integer epoch decay is deterministic and occurs only after a successful maintenance publication. External pressure is consumed as a separate admission envelope, and install/retain hysteresis plus manual-state preservation are enforced.

This closes historical #4 and the remaining runtime-consumer gap of #5 without introducing a second semantic or durable authority.

Recovery now admits optional deferred rebuilds through the same work vocabulary. Initial reopen remains telemetry-independent; after serving, live controller telemetry can prioritize a more valuable advisor-owned rebuild under hard key/semantic-work/byte budgets. Historical #7 remains partial because durable semantic-core checkpoint rehydration and Γ-aware WAL-tail core replay are not yet rebased from the verified R&D branch.

Frozen Pass83 production source delta is limited to `crates/kernel-plan/src/advisor.rs` and `crates/kernel-plan/src/lib.rs`. Full workspace fmt/check/strict-Clippy/tests passed before source freeze; 624 tests are declared and 8 ignored.
