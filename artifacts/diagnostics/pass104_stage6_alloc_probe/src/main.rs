...
fn setup(rows: i64) -> Fixture {
    ...
    let query = match std::env::var("CFMD_QUERY_MODE").as_deref() {
        Ok("scan") => RelExpr::Scan(left),
        Ok("filter") => filtered.clone(),
        Ok("project") => projected.clone(),
        Ok("join") => joined.clone(),
        Ok("group") => grouped,
        _ => topk,
    };
